...
- herdr 0.9.0 or newer, on macOS or Linux. Windows is not supported yet.
...
**Sessions.** Press `prefix+s`. Type to filter. Enter opens the selected session in a split. `Esc` switches from typing to keys: `v` split right, `d` split down, `t` new tab, `c` this pane, `i` paste the session's address into your pane. `s`, `a`, `r` jump between the search, ask, and replies panels.
